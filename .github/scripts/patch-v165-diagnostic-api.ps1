...
# --- Privileged broker: typed Windows activation probe ---
...
function Get-WindowsActivationStatus {
    ...
    try {
        ...
        Write-BrokerLog ("ACTIVATION_CIM_WARN error={0}" -f $_.Exception.Message)
    }
    ...
    try {
        ...
        if($null -ne $activationKey.PSObject.Properties['ProductActivationResult']){
            $activationResult=[int]$activationKey.ProductActivationResult
        }
    } catch {
        Write-BrokerLog ("ACTIVATION_REGISTRY_WARN error={0}" -f $_.Exception.Message)
    }
    ...
    if($null -ne $selected){
        ...
        $activated=($licenseStatus -eq 1)
        $source='SoftwareLicensingProduct'
        $name=[string]$selected.Name
        $description=[string]$selected.Description
        $partialKey=[string]$selected.PartialProductKey
        $reason=$selected.LicenseStatusReason
        $grace=$selected.GracePeriodRemaining
    } elseif($null -ne $activationResult){
        $activated=($activationResult -eq 0)
        $licenseStatusText=if($activated){'Licensed'}else{'NotLicensedOrError'}
        $source='SoftwareProtectionPlatformRegistry'
    }
    ...
    [pscustomobject]@{
        ...
        GracePeriodRemainingMinutes=$grace
        ActivationResult=$activationResult
        EvidenceSource=$source
        WindowsName=if($os){[string]$os.Caption}else{$null}
        WindowsVersion=if($os){[string]$os.Version}else{$null}
        BuildNumber=if($os){[string]$os.BuildNumber}else{$null}
        ProductName=$name
        Channel=$channel
...
$t=Replace-Once $t "        'GetRegistryRead' {`n            return Get-RegistryRead -Path ([string](Get-BrokerParameter -Parameters `$Parameters -Name 'Path' -Default '')) -Name ([string](Get-BrokerParameter -Parameters `$Parameters -Name 'Name' -Default ''))`n        }" "        'GetRegistryRead' {`n            return Get-RegistryRead -Path ([string](Get-BrokerParameter -Parameters `$Parameters -Name 'Path' -Default '')) -Name ([string](Get-BrokerParameter -Parameters `$Parameters -Name 'Name' -Default ''))`n        }`n        'GetWindowsActivationStatus' { return Get-WindowsActivationStatus }" 'activation dispatch'
if(($t -split "GetWindowsActivationStatus").Count -lt 5){ throw 'Activation action was not wired into broker allowlists/dispatch.' }
Write-Utf8 $path $t

# --- Broker client: allow typed read-only activation action ---
...
# --- Stable diagnostic facade copied into each task workspace ---
$diag=@'
param(
...
# --- Agent: copy facade, prefetch activation evidence, direct Codex to typed diagnostics, cleanup runtime dirs ---
...
function Invoke-SessionRuntimeCleanup {
    ...
    foreach($runtimeDir in @($codexSessionTemp,$taskCodexHome)){
        try {
            if(Test-Path -LiteralPath $runtimeDir){Remove-Item -LiteralPath $runtimeDir -Recurse -Force -ErrorAction Stop;$removed += (Split-Path -Leaf $runtimeDir)}
        } catch {$warnings += ((Split-Path -Leaf $runtimeDir)+': '+$_.Exception.Message)}
    }
    try {
        if($null -ne $brokerProcess -and -not $brokerProcess.HasExited){
            try {$brokerProcess.WaitForExit(2500)|Out-Null} catch {}
        }
    } catch {}
...
# Parse all PowerShell sources touched or added.
