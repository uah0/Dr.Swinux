...
$old=@'
foreach($argument in @('exec','--config','approval_policy="never"','--config','windows.sandbox="unelevated"','--sandbox','workspace-write','--cd',$session,'--skip-git-repo-check','--output-last-message',$finalPath,'-'))
'@
$new=@'
...
foreach($needle in @('sandbox_workspace_write.exclude_tmpdir_env_var=true','windows.sandbox="unelevated"','workspace-write','TASK_CODEX_HOME=','CHILD_TMPDIR=')){if(-not $check.Contains($needle)){throw "Invariant missing: $needle"}}
